...
*Note that the terraform scripts [zip file](https://github.com/oci-hpc/oci-hpc-runbook-namd/tree/master/Resources/namd-2.13-cuda.zip) provided in this github already contain object storage urls for NAMD 2.13 CUDA, VMD 1.9.3 and example NAMD models (apoa1 & stmv). If you want to change these url's, modify the variable.tf file and replace the values for namd_url, model_url and vmd_url with your own pre-authenticated request urls.*
  ...
  5. Select your NAMD installer tar file and click <img src="https://github.com/oci-hpc/oci-hpc-runbook-shared/blob/master/images/upload_object.png" height="20">.
...
In the OCI console, there is a Resource Manager available that will create all the resources needed. The region in which you create the stack will be the region in which it is deployed.
  ...
  3. Download the [zip file](https://github.com/oci-hpc/oci-hpc-runbook-namd/tree/master/Resources/namd-2.13-cuda.zip) for terraform scripts and upload it into the stack. 
